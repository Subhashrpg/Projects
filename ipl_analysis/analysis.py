...
# reading data sets
df1 = file_open(r"ipl_datasets\ipl1.csv")
df2 = file_open(r"ipl_datasets\ipl2.csv")
...
def bowler_analysis(bowler):
    try:
        ...
        three_wicket = wicket_df[wicket_df["isWicketDelivery"] == 3]["isWicketDelivery"].count()
...
def bowler_overall(bowler):
    ...
    three_wicket = best_inning[best_inning.isWicketDelivery == 3].groupby("BattingTeam").isWicketDelivery.count().reset_index()
    ...
    economy = round(final_df.apply(lambda row : row["Runs"] / row["Balls"] if row["Balls"] != 0 else "-", axis = 1)*6,2).values
    avg = round(final_df.apply(lambda row : row["Runs"] / row["Wickets"] if row["Wickets"] != 0 else "-", axis = 1),2).values

    final_df.insert(6, "Economy",economy)
    final_df.insert(7, "Average",avg)
    final_df = final_df.set_index("Teams").fillna("-")
